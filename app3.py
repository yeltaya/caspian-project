...
# --- СПЕЦИАЛЬНЫЙ БЛОК Тобол-Торгайского ВХБ ---
    ...
    def show_water_resources_block():
        st.write("---")
        st.header("🌊 Оценка изменения стока рек Казахстана до 2050 года")
        
        # Создание двух колонок: для методологии и для сценариев
        col_method, col_scenarios = st.columns([1, 1], gap="large")
        
        with col_method:
            st.subheader("📊 Материалы и методы исследования")
            st.markdown(f"""
            Исследование базируется на комплексном анализе гидрометеорологических параметров:
            * **Данные наблюдений**: Использование данных государственной наблюдательной сети Казахстана.
            * **Климатические архивы**: Глобальный архив **Terra Climate**.
            * **Прогнозные модели**: Данные по осадкам, рассчитанные по **23 моделям** общей циркуляции атмосферы и океана (МОЦАО) до 2050 г..
        ...
        with col_scenarios:
            st.subheader("🌡️ Климатические сценарии (RCP)")
            
            # Визуализация сценариев с помощью стилизованных блоков
            st.markdown("""
            <div style="display: flex; flex-direction: column; gap: 10px;">
                <div style="background-color: #ff4b4b; padding: 10px; border-radius: 10px; color: white;">
                    <strong>🔴 Сценарий RCP 8.5 («Жесткий»)</strong><br>
                    Быстрый рост антропогенных выбросов и значительное повышение глобальной температуры.
                </div>
                <div style="background-color: #ffa500; padding: 10px; border-radius: 10px; color: white;">
                    <strong>🟠 Сценарий RCP 4.5 («Умеренно жесткий»)</strong><br>
                    Умеренно быстрый рост выбросов и соответствующее повышение температуры.
                </div>
                <div style="background-color: #28a745; padding: 10px; border-radius: 10px; color: white;">
                    <strong>🟢 Сценарий RCP 2.6 («Мягкий»)</strong><br>
                    Замедление роста выбросов с последующим снижением и менее значительное потепление.
                </div>
            </div>
            """, unsafe_allow_html=True)

        # Дополнительная информация по бассейнам (пример на основе Балкаш-Алакольского бассейна)
        st.info("💡 **Прогноз**: В результате оценки изменения стока к 2050 г. в отдельных бассейнах (например, Балкаш-Алакольском) ожидается изменение относительно нормы.")
