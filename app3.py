...
# --- 4. ДЕТАЛЬНАЯ СТАТИСТИКА ---
    ...
    # 1. Заголовок и вводное определение
    ...
    # 2. База данных индексов и их описаний
    index_info = {
        "GDD (Grow)": "🌱 **Градусо-дни роста.** Показывает накопленное тепло, необходимое для созревания культур (пшеницы, кукурузы).",
        
        "GSL": "📅 **Продолжительность вегетационного периода.** Время от последних весенних заморозков до первых осенних. Увеличение GSL позволяет выращивать более теплолюбивые культуры, но требует больше воды.",
        
        "WSDI": "🔥 **Индекс продолжительности тепловой волны.** Критичен для «запала» зерновых. Если жара стоит слишком долго, урожайность падает, даже если влаги в почве достаточно.",
        
        "txge30": "☀️ **Дни с температурой ≥ 30°C.** Порог, при котором у многих культур замедляется фотосинтез и наступает тепловой стресс."
    }

    # 3. Интерфейс выбора
    col1, col2 = st.columns([1, 2]) # Левая колонка чуть уже

    with col1:
        st.subheader("Выбор показателя")
        selected_label = st.radio(
            "Выберите индекс для анализа по РК:",
            options=list(index_info.keys())
        )

    with col2:
        st.subheader("Описание и риски")
        # Отображаем описание в зависимости от выбора
        st.info(index_info[selected_label])

    st.divider()

    # 4. Блок визуализации (заглушки для ваших карт и графиков)
    st.subheader(f"📊 Визуализация для {selected_label}")

    tab1, tab2 = st.tabs(["🗺️ Карта Казахстана", "📈 График динамики"])

    with tab1:
        st.image("https://via.placeholder.com/800x400.png?text=Карта+индекса+по+регионам+РК", 
                 caption=f"Распределение {selected_label} по территории Казахстана")

    with tab2:
        st.image("https://via.placeholder.com/800x300.png?text=График+изменения+по+годам", 
                 caption=f"Тренд {selected_label} (среднее по РК)")

    # 5. Специфический блок, который "Всегда отображен" (согласно вашим настройкам)
    st.sidebar.warning("⚡ **Важно:** Данные индексы являются критическими для продовольственной безопасности Казахстана.")
