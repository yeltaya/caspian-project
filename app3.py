...
with tabs[3]:
    ...
    # --- НАСТРОЙКИ ДАННЫХ ---
    # Словарь соответствия областей и их характеристик
    risk_data = {
        "Акмолинская": {"risk": "Высокий", "color": "#ef6c00", "desc": "Высокий риск из-за глубокого промерзания почвы и больших снегозапасов."},
        "Северо-Казахстанская": {"risk": "Высокий", "color": "#ef6c00", "desc": "Ожидается интенсивное снеготаяние и разлив малых рек."},
        "Карагандинская": {"risk": "Высокий", "color": "#ef6c00", "desc": "Риск формирования склонового стока и ледовых заторов."},
        "Восточно-Казахстанская": {"risk": "Высокий", "color": "#ef6c00", "desc": "Горные реки с риском паводков во всех трех фазах."},
        "Абай": {"risk": "Высокий", "color": "#ef6c00", "desc": "Значительное переувлажнение почвы с осени."},
        "Костанайская": {"risk": "Средний", "color": "#fbc02d", "desc": "Средний риск, возможны локальные подтопления."},
        "Западно-Казахстанская": {"risk": "Средний", "color": "#fbc02d", "desc": "Зависит от трансграничного стока реки Урал."},
        "Актюбинская": {"risk": "Средний", "color": "#fbc02d", "desc": "Возможен быстрый подъем воды на малых реках."},
        "Улытау": {"risk": "Средний", "color": "#fbc02d", "desc": "Прогноз умеренный, требуется мониторинг русел."},
        "Павлодарская": {"risk": "Средний", "color": "#fbc02d", "desc": "Риск зависит от объемов сбросов водохранилищ."},
        "Туркестанская": {"risk": "Средний", "color": "#fbc02d", "desc": "Угроза преимущественно в предгорных районах."},
        "Алматинская": {"risk": "Средний", "color": "#fbc02d", "desc": "Средний риск в период таяния среднегорья."},
        "Жетісу": {"risk": "Средний", "color": "#fbc02d", "desc": "Контроль состояния горных рек в мае-июне."},
        "Атырауская": {"risk": "Низкий", "color": "#66bb6a", "desc": "Низкие показатели осеннего увлажнения."},
        "Мангыстауская": {"risk": "Низкий", "color": "#66bb6a", "desc": "Минимальный риск паводковых явлений."},
        "Кызылординская": {"risk": "Низкий", "color": "#66bb6a", "desc": "Стабильный ледовый режим, риски минимальны."},
        "Жамбылская": {"risk": "Низкий", "color": "#66bb6a", "desc": "Прогноз в пределах многолетних норм."}
    }

    # --- ЗАГРУЗКА ГЕОДАННЫХ ---
    @st.cache_data
    def load_map():
        # Загружаем ваш shp файл
        gdf = gpd.read_file("kaz 17 obl.shp")
        # Важно: убедитесь, что названия в колонке с именами (например, 'NAME_1') 
        # совпадают с ключами в словаре risk_data.
        # Если названия в shp на английском или другом языке, нужно сделать переименование.
        return gdf

    try:
        kaz_gdf = load_map()

        st.subheader("🗺️ Интерактивная карта паводковых рисков")
        st.info("Нажмите на область, чтобы увидеть краткое описание ситуации.")
        ...
        folium.GeoJson(
            ...
            style_function=lambda feature: {
                'fillColor': risk_data.get(feature['properties'].get('NAME_RU'), {}).get('color', '#808080'),
                'color': 'black',
                'weight': 1,
                'fillOpacity': 0.7,
            },
            tooltip=folium.GeoJsonTooltip(fields=['NAME_RU'], aliases=['Область:']),
            popup=folium.GeoJsonPopup(fields=['NAME_RU'], aliases=['Регион:']) # Здесь можно добавить и описание, если объединить данные в GDF
        ).add_to(m)
        ...
        st.error(f"Ошибка загрузки shp-файла: {e}. Проверьте наличие всех файлов (.dbf, .prj, .shx) в папке.")
