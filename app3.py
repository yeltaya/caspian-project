...
# --- 4. ДЕТАЛЬНАЯ СТАТИСТИКА ---
    ...
    # --- 1. ЗАГОЛОВОК И ОБЩЕЕ ОПРЕДЕЛЕНИЕ ---
    ...
    # --- 2. БАЗА ДАННЫХ ПО СЕКТОРАМ И ИНДЕКСАМ ---
    # Структура: Сектор -> { Индекс: Описание }
    data = {
        "Сельское хозяйство": {
            "GDD (Grow)": "🌱 **Градусо-дни роста.** Показывает накопленное тепло, необходимое для созревания культур (пшеницы, кукурузы).",
            "GSL": "📅 **Продолжительность вегетационного периода.** Время от последних весенних заморозков до первых осенних. Позволяет выращивать более теплолюбивые культуры.",
            "WSDI": "🔥 **Индекс длительности тепловой волны.** Критичен для «запала» зерновых. Падение урожайности при долгой жаре.",
            "txge30": "☀️ **Дни с температурой ≥ 30°C.** Порог, при котором у многих культур замедляется фотосинтез."
        },
        "Энергетика": {
            "HDD": "❄️ **Градусо-дни отопления.** Показывает, сколько энергии потребуется на обогрев зданий зимой. В РК этот показатель падает с потеплением.",
            "CDD": "🔌 **Градусо-дни охлаждения.** Растет, показывая резкое увеличение спроса на электричество для кондиционирования летом.",
            "FD": "🧊 **Морозные дни (T min < 0°C).** Важны для мониторинга рисков обледенения линий электропередач."
        },
        "Здравоохранение": {
            "TR": "🌃 **Тропические ночи (T min > 20°C).** Показывает, когда организм человека не успевает восстановиться ночью после дневной жары.",
            "hwd / hwf": "🚑 **Волны жары.** Длительность и частота периодов, наиболее опасных для сердечно-сосудистой системы.",
            "txge30 (Health)": "🌡️ **Экстремальная жара.** Дни, когда резко возрастает количество вызовов скорой помощи."
        },
        "Водное хозяйство и ЧС": {
            "CDD (Dry)": "🏜️ **Продолжительность сухого периода.** Максимальное число дней без осадков. Индикатор засухи.",
            "tnltm2 / tnltm20": "📉 **Ночи с экстремальным холодом.** Влияют на промерзание почвы и риски весенних паводков."
        }
    }
    ...
    with col_sidebar:
        ...
        # Выбор сектора
        selected_sector = st.selectbox("1. Выберите сектор:", list(data.keys()))
        
        # Выбор индекса (зависит от выбранного сектора)
        indices_in_sector = list(data[selected_sector].keys())
        selected_index = st.radio("2. Выберите индекс:", indices_in_sector)

    with col_main:
        st.subheader(f"Анализ: {selected_index}")
        # Вывод динамического описания
        st.info(data[selected_sector][selected_index])
        
        # Блок визуализации
        tab1, tab2 = st.tabs(["🗺️ Карта Казахстана", "📈 График динамики"])
        
        with tab1:
            # Здесь вы можете подставлять путь к файлу динамически: f"maps/{selected_index}.png"
            st.image("https://via.placeholder.com/800x400.png?text=Карта+РК", 
                     caption=f"Пространственное распределение {selected_index} по территории РК")
        
        with tab2:
            st.image("https://via.placeholder.com/800x300.png?text=График+тренда", 
                     caption=f"Изменение показателя {selected_index} по годам")

    # --- 4. БЛОК "ВСЕГДА НА ЭКРАНЕ" ---
    st.sidebar.markdown("---")
    st.sidebar.warning(f"**Текущий контекст:** {selected_sector}")
    st.sidebar.write("Данные показатели рассчитываются для оценки реальных экономических рисков РК.")
