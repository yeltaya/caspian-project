...
# Добавляем разделитель или небольшой отступ
    ...
    # Используем st.expander (аккордеон), чтобы скрыть детали и сделать интерфейс чище
    ...
    with st.expander("💧 Гидрологический мониторинг (HydroSoS)"):
        st.markdown("""
    ...
    with st.expander("📡 Интегрированная система ВМО (WIGOS)"):
        st.markdown("""
        **Совместный Региональный Центр WIGOS (Суб-регионы VI Европа и II Азия):**
        * Пилотный проект, реализуемый НГМС стран Казахстана, России и Беларуси (на основе ротации).
        * Повышение качества метеорологических наблюдений и обмена данными.
        """)

    with st.expander("📢 Глобальная инициатива «Раннее предупреждение для всех» (EW4All)"):
        # Используем st.info или цветную плашку, так как это лидерская роль
        st.info("""
        **Ключевая роль НГМС РК:**
        * Выступает в качестве **Председателя Целевой группы** для Региона II (Азия).
        * Региональный участник в Восточной Европе и Центральной Азии.
        """)

    # Стилизация (опционально, добавьте в ваш блок <style>)
    st.markdown("""
        <style>
        .stExpander {
