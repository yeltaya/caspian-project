...
# --- 4. ДЕТАЛЬНАЯ СТАТИСТИКА ---
    ...
    # Настройка страницы
    st.set_page_config(page_title="Климатический мониторинг Казахстана", layout="wide")

    # --- ВВОДНАЯ ЧАСТЬ (Ваше описание) ---
    st.title("🌍 Климатические индексы и экономика Казахстана")

    # Выделяем определение в блок для акцента
    st.markdown("""
    ...
    > 
    > В отличие от простых метеорологических величин (например, среднесуточной температуры), индекс базируется на **пороговых значениях**, имеющих критическое значение для физических, биологических или экономических систем.
    """)

    st.markdown("---")

    # --- ОСНОВНОЙ ИНТЕРФЕЙС ---
    # (Далее идет выбор секторов, графики и карты)

    col_sector, col_index = st.columns(2)

    with col_sector:
        sector = st.selectbox("Выберите сектор экономики:", 
                              ["Сельское хозяйство", "Энергетика", "Здравоохранение", "Водное хозяйство", "Лесное хозяйство"])

    # Пример наполнения для Сельского хозяйства
    if sector == "Сельское хозяйство":
        with col_index:
            index = st.radio("Выберите индекс для анализа:", ["GDD", "GSL", "WSDI", "txge30"])
        
        st.subheader(f"Анализ индекса {index}")
        
        # Блок с описанием конкретного индекса (будет меняться динамически)
        if index == "GDD":
            st.write("**GDD (Grow):** Градусо-дни роста. Показывает накопленное тепло, необходимое для созревания культур (пшеницы, кукурузы).")
        elif index == "GSL":
            st.write("**GSL:** Продолжительность вегетационного периода. Увеличение GSL позволяет выращивать теплолюбивые культуры.")

        # Ряд для графиков и карт
        c1, c2 = st.columns(2)
        with c1:
            st.info(f"Здесь отображается Карта РК по индексу {index}")
            # st.image("path_to_map.png") 
        with c2:
            st.info(f"Здесь отображается График тренда по годам для {index}")
            # st.line_chart(data)
