...
# Пропорции колонок
    ...
    def load_climate_data():
        # 1. Загрузка температуры
        df_temp = pd.read_csv("Summary_Anom_T_1941-2025.xlsx - Temp.csv")
        
        # 2. Загрузка осадков
        df_precip = pd.read_csv("Summary_Anom_R_1941-2025.xlsx - Sheet1.csv")

        # Словарь для приведения разных названий из CSV к вашим старым ключам
        # Температурный файл использует форматы: АТЫРАУ.ОБЛ
        # Файл осадков использует форматы: Атыр обл
        rename_temp = {
            'АТЫРАУ.ОБЛ': 'Атырау', 'МАНГИС.ОБЛ': 'Мангистау', 'АКТЮБИН.ОБЛ': 'Актюбинская',
            'КАРАГ.ОБЛ': 'Карагандинская', 'УЛЫТАУ.ОБЛ': 'Улытауская', 'АБАЙ.ОБЛ': 'Абай',
            'АКМОЛ.ОБЛ': 'Акмола', 'КОСТ.ОБЛ': 'Костанай', 'ПАВЛ.ОБЛ': 'Павлодар',
            'АЛМАТИН.ОБЛ': 'Алматинская', 'ЖЕТЫСУ.ОБЛ': 'Жетысу', 'ТУРКЕСТ.ОБЛ': 'Туркестанская',
            'КЫЗЫЛ.ОБЛ': 'Кызылординская', 'ЖАМБЫЛ.ОБЛ': 'Жамбылская'
        }

        rename_precip = {
            'Атыр обл': 'Атырау', 'Мангист обл': 'Мангистау', 'Актю обл': 'Актюбинская',
            'Караг обл': 'Карагандинская', 'Улытау обл': 'Улытауская', 'Абайск.обл': 'Абай',
            'Акм обл': 'Акмола', 'Кост обл': 'Костанай', 'Павл обл': 'Павлодар',
            'Алмат обл': 'Алматинская', 'Жетысус обл': 'Жетысу', 'Турк обл': 'Туркестанская',
            'КЗО': 'Кызылординская', 'жамб обл': 'Жамбылская'
        }

        df_temp = df_temp.rename(columns=rename_temp)
        df_precip = df_precip.rename(columns=rename_precip)
        
        return df_temp, df_precip

    # Загружаем данные
    try:
        df_temp, df_precip = load_climate_data()
        st.success("Данные по температуре и осадкам успешно загружены!")
    except Exception as e:
        st.error(f"Ошибка при загрузке файлов: {e}")

    # Пример использования в интерфейсе:
    option = st.radio("Выберите показатель:", ["Температура", "Осадки"])
    ...
    if option == "Температура":
        ...
        df_active = df_precip
        label = "Отклонение осадков (%)"

    # Теперь df_active можно использовать для графиков Plotly
    # Например: st.line_chart(df_active.set_index('Год')['ЗКО'])
