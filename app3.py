...
#МОЖНО СДЕЛАТЬ СРАВНЕНИЕ ПО ГОРОДАМ 
  

    # 1. Формируем базу данных на основе вашего текста
    water_data = [
        # 2025 год
        {"Год": 2025, "Объект": "р. Сарыбулак", "Класс": 6, "Статус": "Высоко загрязненная", 
         "Инфо": "1 случай ЭВЗ (кислород), 29 случаев ВЗ. Вода не рекомендована для купания и орошения."},
        {"Год": 2025, "Объект": "р. Акбулак", "Класс": 6, "Статус": "Высоко загрязненная", 
         "Инфо": "Загрязнение хлоридами. Пригодна только для тех. нужд."},
        {"Год": 2025, "Объект": "Астанинское вдхр.", "Класс": 1, "Статус": "Очень хорошее", 
         "Инфо": "Пригодна для всех видов водопользования."},
        
        # 2024 год
        {"Год": 2024, "Объект": "р. Акбулак", "Класс": 5.5, "Статус": "Грязная", 
         "Инфо": "12 случаев ЭВЗ и 3 случая ВЗ по растворенному кислороду."},
        {"Год": 2024, "Объект": "р. Сарыбулак", "Класс": 5.5, "Статус": "Грязная", 
         "Инфо": "Превышение по хлоридам. Не пригодна для использования."},
         
        # 2023 год
        {"Год": 2023, "Объект": "р. Акбулак", "Класс": 5.5, "Статус": "Грязная", 
         "Инфо": "25 случаев ЭВЗ по растворенному кислороду. Направлено в КЭРК."},
        {"Год": 2023, "Объект": "р. Сарыбулак", "Класс": 5.5, "Статус": "Грязная", 
         "Инфо": "21 случай ВЗ по хлоридам, магнию и минерализации."}
    ]

    df = pd.DataFrame(water_data)

    st.set_page_config(layout="wide")
    st.title("💧 Интерактивная карта качества вод Астаны")

    # Выбор года
    year_to_filter = st.selectbox("Выберите год мониторинга:", [2025, 2024, 2023])

    # Фильтрация
    filtered_df = df[df['Год'] == year_to_filter]

    # Создание графика с кастомными подсказками
    fig = px.bar(
        filtered_df, 
        x="Объект", 
        y="Класс",
        color="Статус",
        # Цветовая схема: от зеленого (хорошо) до черного (критично)
        color_discrete_map={
            "Очень хорошее": "#2ecc71",
            "Умеренно загрязненные": "#f1c40f",
            "Грязная": "#e67e22",
            "Высоко загрязненная": "#c0392b"
        },
        title=f"Классы качества водных объектов ({year_to_filter} г.)",
        # НАСТРОЙКА TOOLTIPS (Подсказок)
        hover_name="Объект", 
        hover_data={
            "Класс": True,
            "Статус": True,
            "Инфо": True, # Выводит ваш текст
            "Год": False   # Скрываем лишнее
        }
    )
    ...
    # Информационный блок
    if year_to_filter == 2025:
        st.warning("**Важное замечание по 2025 году:** На р. Сарыбулак зафиксировано экстремальное загрязнение (ЭВЗ) по кислороду. Это критический показатель для экосистемы.")
    ...
    col_evz, col_vz, col_indicators = st.columns(3)

    with col_evz:
        st.metric(label="Случаи Экстремально Высокого Загрязнения (ЭВЗ)", value="1 случай", delta="Кислород", delta_color="inverse")
    with col_vz:
        st.metric(label="Случаи Высокого Загрязнения (ВЗ)", value="29 случаев", delta="Аммоний, Фосфор, Хлориды", delta_color="inverse")
    with col_indicators:
        st.info("**Мониторинг:** 17 створов, 37 гидрохимических показателей.")
