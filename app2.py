...
def get_icon_html(icon_type, is_selected=False):
    bg_color = DARK_BLUE if is_selected else LIGHT_BG
    icon_color = "white" if is_selected else DARK_BLUE
    
    # Новые контурные пути для иконок (стилистика Flaticon)
    paths = {
        "Уровень моря": "M2 12c3-2 5 2 8 2s5-4 8-2 M2 16c3-2 5 2 8 2s5-4 8-2",
        "Температура воздуха": "M12 2v14M9 5l3-3 3 3M12 22a3 3 0 1 0 0-6 3 3 0 0 0 0 6z",
        "Температура воды": "M12 20a6 6 0 0 0 6-6c0-4-6-10-6-10S6 10 6 14a6 6 0 0 0 6 6z",
        "Соленость": "M11 2v7.58l-4.43 4.1a2 2 0 0 0-.7 3.51H18a2 2 0 0 0-.7-3.51L13 9.58V2h-2z",
        "Лед": "M12 2l2 4h4l-3 3 1 5-4-3-4 3 1-5-3-3h4l2-4z",
        "Ветер": "M2 8h15a2 2 0 1 0-2-2M2 12h12a2 2 0 1 1-2 2M2 16h18a2 2 0 1 0-2-2",
        "Волнение": "M2 12s3-4 7-4 7 4 11 4M2 16s3-4 7-4 7 4 11 4"
    }
    
    path = paths.get(icon_type, "")
    return f"""
    <div style="display: flex; align-items: center; justify-content: center; 
                width: 40px; height: 40px; background-color: {bg_color}; 
                border-radius: 12px; transition: all 0.3s; border: 1.5px solid {DARK_BLUE if not is_selected else 'transparent'};">
...
def get_header_icon(icon_type):
    paths = {
        "Сеть": "M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z", # Щит/Сеть
        "Динамика": "M3 17l6-6 4 4 8-8M21 13v8h-8", # Тренд
        "История": "M12 8v4l3 3m6-3a9 9 0 1 1-18 0 9 9 0 0 1 18 0z", # Часы
        "Факторы": "M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" # Слои
    }
    path = paths.get(icon_type, "")
    return f"""
    <div style="background-color: {LIGHT_BG}; padding: 8px; border-radius: 10px; border: 1.5px solid {DARK_BLUE}; display: flex; align-items: center; justify-content: center;">
        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" 
             stroke="{DARK_BLUE}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <path d="{path}"/>
        </svg>
    </div>
    """

def get_micro_icon(icon_type):
    paths = {
        "корабль": "M2 18h20l-2-7H4l-2 7zM10 11V7a2 2 0 1 1 4 0v4",
        "облако": "M17.5 19c2.5 0 4.5-2 4.5-4.5 0-2-1.3-3.7-3.2-4.2a6.5 6.5 0 1 0-12.6 1.7C4.2 12.5 3 13.9 3 15.5 3 17.4 4.6 19 6.5 19h11z",
        "капля": "M12 22a7 7 0 0 0 7-7c0-3.8-7-13-7-13S5 11.2 5 15a7 7 0 0 0 7 7z",
        "пробирка": "M10 2v7.58l-4.43 4.1a2 2 0 0 0-.7 3.51h9.44a2 2 0 0 0-.7-3.51L14 9.58V2h-4z"
    }
    path = paths.get(icon_type, "")
    return f"""
    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" 
         stroke="{DARK_BLUE}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 10px;">
        <path d="{path}"/>
    </svg>
    """

# --- 1. НАСТРОЙКА СТРАНИЦЫ И СТИЛЕЙ ---
st.set_page_config(page_title="Исследование Каспия", layout="wide")
...
st.markdown(f"""
    ...
    * {{ font-family: 'Montserrat', sans-serif !important; }}
    ...
    .promo-bold {{ color: {DARK_BLUE}; font-weight: 800; font-size: 1.4rem; margin-bottom: 10px; }}
    .promo-sub {{ color: {GRAY_TEXT}; font-size: 1.05rem; line-height: 1.5; }}
    
    /* Стилизация кнопок под выбранный стиль */
    div.stButton > button {{
...
# --- 3. ОТОБРАЖЕНИЕ ВЕРХНЕГО БЛОКА ---
st.markdown(f'<h1 style="text-align: center; color: {DARK_BLUE}; font-weight: 800; margin-bottom: 40px;">ИССЛЕДОВАНИЕ КАСПИЙСКОГО МОРЯ</h1>', unsafe_allow_html=True)

t_col1, t_col2, t_col3 = st.columns([1, 1, 1.2])

with t_col1:
    # 1. Заголовок (собираем из частей, чтобы избежать ошибок парсинга)
    header_icon = get_header_icon("Сеть")
    header_text = '<span style="color: #001F3F; font-weight: 800; font-size: 1.5rem; margin-left: 10px;">Сеть мониторинга</span>'
    st.markdown('<div style="display: flex; align-items: center; margin-bottom: 15px;">' + header_icon + header_text + '</div>', unsafe_allow_html=True)
    
    # 2. Описание
    st.markdown('<div class="official-text">РГП «Казгидромет» осуществляет непрерывный мониторинг казахстанского сектора Каспийского моря.</div>', unsafe_allow_html=True)
    
    # 3. Список станций с новыми микро-иконками (используем чистый HTML без f-строк)
    network_list_html = f"""
    <div style="background: #F8FAFC; padding: 15px; border-radius: 12px; border-left: 4px solid #001F3F; margin-top: 10px;">
        <div style="display: flex; align-items: center; margin-bottom: 8px;">
            {get_micro_icon("корабль")} <span style="color: #1E293B;"><b>10</b> морских станций</span>
        </div>
        <div style="display: flex; align-items: center; margin-bottom: 8px;">
            {get_micro_icon("облако")} <span style="color: #1E293B;"><b>28</b> метеостанций</span>
        </div>
        <div style="display: flex; align-items: center; margin-bottom: 8px;">
            {get_micro_icon("капля")} <span style="color: #1E293B;"><b>4</b> гидропоста</span>
        </div>
        <div style="display: flex; align-items: center;">
            {get_micro_icon("пробирка")} <span style="color: #1E293B;"><b>50</b> точек качества</span>
        </div>
    </div>
    """
    st.markdown(network_list_html, unsafe_allow_html=True)
    
    
with t_col2:
    # Заголовок блока Параметры
    st.markdown(f'<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 15px;">{get_header_icon("Факторы")}<h2 style="color: #001F3F; font-weight: 800; font-size: 1.5rem; margin: 0;">Параметры</h2></div>', unsafe_allow_html=True)
    
    st.markdown('<div style="color: #64748B; font-size: 0.9rem; margin-bottom: 15px; font-weight: 600;">📅 Оперативные данные за 2025 г.</div>', unsafe_allow_html=True)
    
    params_list = ["Уровень моря", "Температура воздуха", "Температура воды", "Соленость", "Лед", "Ветер", "Волнение"]
    
    for name in params_list:
        is_active = (st.session_state.selected_param == name)
        col_icon, col_btn = st.columns([0.25, 0.75]) # Чуть увеличили место под иконку
        
        with col_icon:
            # Используем контейнер, чтобы иконка не "улетала"
            st.write("") # Микро-отступ для выравнивания
            st.markdown(get_icon_html(name, is_active), unsafe_allow_html=True)
...
with t_col3:
    ...
    # Заголовок блока График
    st.markdown(f'<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 15px;">{get_header_icon("Динамика")}<h2 style="color: #001F3F; font-weight: 800; font-size: 1.5rem; margin: 0;">Сезонный ход ({current_unit})</h2></div>', unsafe_allow_html=True)
    
    # Ваш код графика остается без изменений
    fig_s = go.Figure()
    display_data = seasonal_data.get(st.session_state.selected_param, [0]*12)
    fig_s.add_trace(go.Scatter(
        x=months, y=display_data,
        mode='lines+markers', 
        line=dict(color='#001F3F', width=3, shape='spline'),
        marker=dict(size=8, color='white', line=dict(color='#001F3F', width=2))
    ))
    fig_s.update_layout(height=280, margin=dict(l=10, r=10, t=30, b=10), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
    ...
    with st.expander("🔍 Почему уровень моря постоянно меняется? (Научный контекст)"):
        st.markdown("""
            <div style="padding: 10px; line-height: 1.6; color: #334155;">
                Каспийское море — это замкнутый водоем, его уровень работает как <b>гигантский климатический индикатор</b>. 
...
impact_items = [
    ...
    },
    {
        ...
        "desc": "<b>Береговая линия:</b> Смещение границы воды на километры делает причалы и порты бесполезными.",
